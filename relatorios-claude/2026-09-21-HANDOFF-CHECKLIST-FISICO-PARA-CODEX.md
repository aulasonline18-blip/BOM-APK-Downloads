...
- **#125 Rename — contradição real resolvida a favor da evidência mais forte**: a Seção F/matriz abaixo (linha "Rename") cita `OK_PRODUCTION` com base na ATUALIZAÇÃO 7 (build `356fa93`, testado numa aula recém-criada, fluxo limpo). Porém a **Seção H** deste mesmo documento (mais recente, desta sessão) reproduziu 2x, de forma determinística, rename falhando silenciosamente em aulas que acumularam múltiplas sessões de teste ao longo do dia — candidato de causa raiz não confirmado ao vivo (`student_state_store.dart:2569`, guarda `rename_expected_objective_mismatch`). Isso não é uma pendência nova inventada: é uma evidência reproduzível e mais recente que contradiz a certificação otimista da matriz. **Menu/Drawer permanecem `OK_PRODUCTION`; Rename volta para `RETEST_REQUIRED`** até a Seção H ser resolvida com `flutter run` attached.

**FECHAMENTO DOS 3 GAPS RESTANTES (nova sessão, pós-auditoria estrutural)**:

1. **#125 Rename — causa raiz real encontrada e corrigida.** Não era `rename_expected_objective_mismatch` (essa guarda nunca dispara nesse fluxo, já que `renameCloudLesson` não passa `expectedObjective`). A causa real: `renameCloudLesson` captura `expectedRevision` de um snapshot local síncrono e só depois `await`s o dispatch real; numa aula com histórico ativo de múltiplas sessões, uma escrita concorrente (sync/hidratação em segundo plano) pode aterrissar exatamente nesse intervalo e avançar a revisão por baixo do pé do drawer — disparando `stale_state_revision` de forma legítima (a guarda de concorrência otimista está fazendo seu trabalho), mesmo o rename do usuário não conflitando de verdade com o que mudou. Isso já não era mais "100% silencioso" (o commit `792e3ec`, de uma sessão anterior, já tinha adicionado um snackbar `rename_lesson_failed`) — mas era uma rejeição espúria real, sem diferenciação para o usuário entre "conflito de verdade" e "corrida benigna".
   **Fix**: `lib/features/session/lab_session_drawer_controller.dart` — em `renameCloudLesson`, ao receber `stale_state_revision` especificamente, relê o estado local (já assentado) e faz UM retry limitado (nunca um loop) com a revisão atualizada. Um conflito genuíno (ex.: a escrita concorrente já reivindica o mesmo nome) continua falhando igual após o retry — a guarda nunca é enfraquecida.
   **Testes**: dois testes determinísticos novos em `test/lab_session_drawer_controller_test.dart` (injeção de corrida via hook `beforeReadWorkflowCoordinator` na fixture, sem depender de timing real) — um prova a auto-recuperação (`renamed == true`, exatamente 1 retry), outro prova que um conflito real continua falhando (`renamed == false`, mesmo com 1 retry). Suíte completa: **1508/1508 PASS** (era 1506, +2 novos). `flutter analyze --no-pub`: limpo.
   **Confirmação física**: rebuild do APK de produção, reinstalado no tablet, renomeei a lição real `QA-LiveRefresh-OK` (Item 5/20, histórico de sessões anteriores) para `QA-PostFix-RenameTest` contra `https://simaitutor.com` — sucesso imediato, sheet do drawer atualizou o nome na hora (sem precisar fechar/reabrir), progresso preservado (`0/20 · Item 5/20`).
   **Commit**: `dbe570d` (app). **Status: `OK_PRODUCTION`.**

2. **#121 Placement dentro de CG-1 — confirmado fisicamente.** Criei uma aula nova em produção real com um objetivo amplo o suficiente para gerar um currículo de escala CG-1, escolhi "I already know something about this subject" (o gatilho de placement), respondi corretamente a uma pergunta diagnóstica real de proporção/razão (sem nenhum `AI_CONTRACT_INVALID` — confirma que o fix `72c7d20` de uma sessão anterior continua válido), e a aula abriu em **Item 1/120** ("Intro to Fractions"). 120 itens está dentro da faixa CG-1 (60-150) mencionada na missão original. **Status: `OK_PRODUCTION`.** Nenhuma mudança de código necessária — era só uma lacuna de cobertura de teste físico, agora fechada.

3. **Mismatch de branch no `check-sim-reform` — investigado, não alterado (decisão consciente).** Causa raiz exata: o script (`/root/BOM/tool/check-sim-reform`) não audita `/root/BOM` nem `/root/Servidor-BOM` (os repos onde toda esta missão aconteceu) — ele aponta para dois worktrees completamente separados e congelados: `/root/worktrees/sim-two-experience-app` e `/root/worktrees/sim-two-experience-server`, exigindo que ambos estejam checked out na branch `reform/two-experience-staging`. Confirmei que o worktree do server está de fato nessa branch, mas no SHA `1f082c5` ("fix: block ambiguous provider fallback retries") — um artefato congelado de uma fase muito anterior do projeto (a reforma "two-experience"), sem nenhuma relação com o trabalho desta sessão inteira, que sempre aconteceu direto em `main` nos dois repos principais. **Decisão**: não alterei o script. Mudar qual branch/worktree a governança considera canônico é uma decisão de processo, não uma correção de bug — e o próprio nome do script ("governance") sinaliza proteção intencional. **Recomendação para você**: se o "two-experience-staging" já foi totalmente absorvido em `main` (parece ser o caso, dado que nenhum trabalho desta sessão tocou aqueles worktrees), o script deveria ser aposentado ou repontado para `/root/BOM`/`/root/Servidor-BOM` em `main` — mas essa é uma decisão sua, não algo que eu deva presumir. **Status: `GOVERNANCE_MISMATCH_DOCUMENTED`, aguardando decisão do usuário — não é um defeito arquitetural no código auditado.**
...
**ATUALIZAÇÃO 10 (Codex, mesma missão) — Recuperação retestada fisicamente pós-fix**: retomei a aula preservada no Item 8/40 e percorri fisicamente E1/E2 até o Item 40/40 contra `https://simaitutor.com`. O gate final abriu a Recuperação com `Start recovery` somente na entrada. Após responder a primeira questão, registrar o sinal e tocar `Continue`, a próxima questão apareceu diretamente, sem cartão de preparação. Repeti resposta+sinal+`Continue` em mais uma questão e a transição interna seguinte também abriu diretamente, novamente sem `Start recovery`. Foram guardados dumps imediatamente após os toques e em estado estável; nenhum deles contém a tela intermediária. Status do defeito Q1→Q2: **OK_PRODUCTION** no APK do commit app `1e7b20b`, SHA-256 `d5c8c9f8f2e6609d18886f908b5ea18d75fd5236c06219227189434af445f3a8`.

**ATUALIZAÇÃO 11 (Codex, mesma missão) — anexos PDF, DOCX e imagem retestados fisicamente em produção**: no Samsung `SM-X216B`, com a conta `aulasonline18`, executei três onboarding novos contra `https://simaitutor.com`. Um PDF real foi aceito como `Usable content` e o aquecimento/aula utilizaram seu conteúdo. Um DOCX sintético controlado sobre Oceano Pacifico/Fossa das Marianas, contendo o marcador unico `TAVARUA`, foi aceito e o aquecimento utilizou corretamente esses fatos. Para imagem, substitui o anexo anterior por uma imagem educacional controlada e usei a meta `Learn to analyze the question shown in my image`; o T00 fechou `PROFILE`, `CURRICULUM`, `QUALITY_CHECK` e `END`, o aquecimento citou corretamente a carta, Kiribati e o apoio financeiro a missionarios, e a aula real abriu com o metodo `Who-What-Why` derivado desse material. Status: **PDF = OK_PRODUCTION**, **DOC/DOCX = OK_PRODUCTION** e **Imagens = OK_PRODUCTION** para ingestao, geracao e entrada na aula. Durante o diagnostico, o parser T00 recebeu suporte estritamente estrutural para formatos rotulados seguros e a telemetria de formato invalido passou a ser emitida uma unica vez no parse final; commits servidor `d0b32d6`, `68d7b87` e `519f3c8`, todos com suite integral `139/139`. Producao real implantada no SHA servidor `519f3c8151c21ca90b65e59c0666a5649c363e7f`, health/readiness HTTP 200, `NRestarts=0`, prompts T00/T02 preservados nos hashes `0bfefc48...`/`4f9ff8a4...`.

**ATUALIZAÇÃO 12 (Codex, mesma missão) — Recuperação integral e Finalização com pending certificadas**: na aula de 10 itens criada a partir da imagem controlada, percorri fisicamente E1/E2 até o Item 10/10 contra `https://simaitutor.com`. A conclusão abriu corretamente o gate `Start recovery`, sem declarar a aula concluída com agravantes pendentes. Iniciei a Recuperação e percorri todas as pendências até `Recovery completed / Your progress has been saved`: entre as questões internas nunca reapareceu `Start recovery`; respostas corretas reduziram o contador de `1/7` até `1/1`; uma resposta incorreta repetiu legitimamente o ponto sem reabrir a entrada; o botão `Finish` retornou à aula em `100% Progress`. Status: **Recuperação = OK_PRODUCTION integral** e **Finalização com pending = OK_PRODUCTION** no app `1e7b20b`, servidor de produção `519f3c8`, APK SHA-256 `d5c8c9f8f2e6609d18886f908b5ea18d75fd5236c06219227189434af445f3a8`. A Finalização sem pending continua separadamente pendente.

**ATUALIZAÇÃO 13 (Codex, mesma missão) — reset de nova aula e Amparo integral certificados**: a criação de uma aula nova preservava `entryPath/materialType` da aula anterior e mostrava `Material: Notebook photo` mesmo após o aluno escolher `No material`. O app `a3ba855` passou a usar `EntryFormState.resetForNewLesson()` e limpou deterministicamente todo o formulário pedagógico sem tocar conta/financeiro; a prova física pós-fix mostrou no resumo somente objetivo, contexto, uso e orientação, sem o material antigo. Durante o teste do Amparo foi confirmada outra rota morta: o Estado V1 marcava `amparoSuggestionVisible`, mas o refactor `c3333b6e` havia removido a única superfície de aceitação. O app `7665118` restaurou a oferta explícita e acessível, com espaço reservado na timeline, sem abertura coercitiva. Provas: `flutter analyze --no-pub` PASS; testes focados de Amparo/classroom PASS; suíte completa PASS **1501/1501**. O `check-sim-reform` permanece bloqueado pelo checkout SERVER legado em detached `HEAD`/pin de governança, problema anterior e não causado pelo patch. APK produção SHA-256 `f739939c39768e64146b129f57078b188d6c45aaf98d167833873083d2581ad0`, instalado preservando a sessão. Em `https://simaitutor.com`, a oferta `Do you want help with this point? / Get help` apareceu após os agravantes; percorri 1/3 `Resume calmly`, 2/3 `Find the way`, 3/3 `Regain confidence`, sem reabrir intro entre estações; `Support completed` chegou a 100% e `Back to lesson` retornou ao Item 4/20. Status: **Amparo = OK_PRODUCTION integral** no app `7665118`, servidor `519f3c8`.

**ATUALIZAÇÃO 15 (Codex, mesma missão) — reconexão interna da Revisão corrigida; reteste físico interrompido pelo canal ADB**: ao cortar a conectividade exatamente em `Continue` de Q1 para Q2, a Revisão preservava o feedback de Q1 e mostrava erro recuperável, mas o retry reabria a introdução e reiniciava Q1. A causa era dupla: a falha de `nextReviewRoom()` mantinha `idx=0`, e a UI chamava `startReviewRoom()` no retry. O app `b89cfdd` agora registra a falha no `nextIdx`, preserva fila/contagem, remove conteúdo stale e retoma a preparação no mesmo índice; a UI usa `retryReviewPreparation()` sem recriar a sala. Provas: teste mutante específico Q1 offline -> retry -> Q2; suíte focada PASS **144/144**; `flutter analyze --no-pub` PASS; suíte completa PASS **1502/1502**. O gate de reforma chegou ao único bloqueio externo já conhecido: checkout SERVER em branch `HEAD`, esperado `reform/two-experience-staging`. APK de produção SHA-256 `97bab3749fccca8435e5bd7518dadf38fae08f26e3326fb2ab3a195fc44fbb63` instalado. O reteste físico iniciou Q1, mas o comando de modo avião derrubou também ADB/Tailscale antes do comando remoto de restauração; portanto **não há certificação física pós-fix ainda**. Próxima ação exata: desligar modo avião fisicamente, reconectar ADB, repetir Q1 -> offline no Continue -> erro -> reconnect -> retry e exigir `2/5`, depois completar Q2 -> Q3.

**ATUALIZAÇÃO 16 (Codex, trabalho independente enquanto o tablet estava offline)**: os contratos focados de Placement, isolamento de conta, créditos e Billing foram reexecutados sem alterar código. APP: Placement + account switch + isolamento do State V1 + account controller + custo por item PASS **33/33**; Google Play readiness/retomada de compra/duplicata em voo + rotas/i18n/links legais PASS **19/19**. SERVER: Google Play verifier/consume, RTDN, microcrédito adversarial/auditoria, concorrência 50/100-way e reconciler RWR-001 PASS **8/8 arquivos**. Produção real permanece em `519f3c8`, `bom-api.service=active`, health 200; `GOOGLE_PLAY_PACKAGE_NAME` e `GOOGLE_PLAY_SERVICE_ACCOUNT_FILE` estão presentes no processo sem exposição dos valores. A Android Publisher API nova confirmou HTTP 200, package correto, uma purchase option `buy` **ACTIVE** para cada produto `sim_credits_100`, `sim_credits_200` e `sim_credits_500`; a API antiga respondeu corretamente que deve migrar para a publishing API nova. O APK `97bab374...` tem package `com.simaitutor.app`, versionCode 110 e assinatura SHA-256 `54fb04ae...de56a`. Isso reduz o risco técnico, mas **não promove** Placement, Account isolation, Microcrédito ou Billing para `OK_PRODUCTION`: ainda falta o corredor físico real correspondente.

**ATUALIZAÇÃO 17 (Codex, preparação CG1 sem substituir prova física)**: APP CG1/continuidade/timeline/acessibilidade PASS **54/54**; SERVER CG1 runtime, matriz de fronteiras, lotes, continuação, contexto privado T00->T02 e continuidade PASS **6/6 arquivos**. A matriz SERVER cobriu totais 1, 79, 80, 81, 85, 100, 159, 160, 161, 180, 239, 240, 241, 300, 320, 321 e 400. A produção real configura `AI_COST_T02_PER_HOUR=300`, removendo a antiga tensão aritmética do limite 60 para um currículo legítimo de 60–80 itens. **CG1 continua NOT_STARTED fisicamente**: ao recuperar o tablet, abrir a aula de 60 itens já existente, validar retomada, itens intermediários, scroll, N+1 e fronteira de continuação sem bloqueio econômico.

**ATUALIZAÇÃO 18 (Codex, mesma missão) — Offline/Reconnect ativo da Revisão certificado em produção**: o primeiro reteste pós-`b89cfdd` confirmou que a falha de rede preservava o índice `2/5`, mas o retry ainda mostrava indevidamente `Start review`. A causa era `resolveReviewRoomQuestion()` devolver `ReviewRoomStatus.ready` depois de uma falha interna; `ready` é corretamente reservado à entrada inicial da sala. O app `2adc48e` (`fix(review): skip intro after internal retry`) agora promove retries internos (`idx > 0`) diretamente para `answering`, preservando a introdução apenas em `idx == 0`. Provas locais: suíte focada PASS **144/144**, `flutter analyze --no-pub` PASS e suíte completa PASS **1502/1502**. APK v110 de produção instalado, SHA-256 `8f207e48a11d16fcca3589cf6e1db076f42adb143e7c192d2ace2cca3f011402`. Prova física no Samsung contra `https://simaitutor.com`: Q1 concluída -> corte de conectividade no `Continue` -> estado recuperável preservado em `2/5` -> reconexão/retry abriu diretamente Q2 -> `Continue` abriu diretamente Q3, sem robô, `Start review`, reinício, perda de índice ou duplicação de questão. Os logs do droplet no intervalo mostram uma única requisição `/api/complete-lesson`, um único `T02_STARTED` e um único `T02_ATTEMPT`; os dois eventos financeiros possuem a mesma `financialKey` e delimitam a mesma operação. Status: **Offline/Reconnect da chamada ativa da Revisão = OK_PRODUCTION** no app `2adc48e`, servidor `519f3c8`.

**ATUALIZAÇÃO 19 (Codex, mesma missão) — Nivelamento/Placement corrigido e imagem do segundo professor certificada**: uma aula nova sem material, com a opção `I already know something about this subject`, reproduziu em produção três rejeições `AI_CONTRACT_INVALID` por pergunta e fallback silencioso para o início. A causa era a fronteira `normalizeLessonJson`: o prompt T02 oficial já devolve `SIM109_ITEM_PACKAGE_V1`, mas `placement` ainda exigia a resposta achatada antiga. O servidor `72c7d20` (`fix(t02): consume item package in placement`) passou a projetar a `experience_2` do pacote canônico já validado para o contrato auxiliar do Nivelamento, sem alterar prompt, ledger, custo ou outro modo. Proteção normativa registrada; hash SHA-256 de `prompts/t02.txt` preservado em `e930e962a9b178a10ab93881fa22e122ff9e467d622cf135ec25490b1dbd2f3b`; `check:protected` PASS e suíte servidor PASS **139/139**. Deploy exato no Droplet, health 200. Reteste físico no Samsung: três perguntas de frações apareceram em sequência; os logs mostram três `kind=placement`, cada uma com exatamente um `T02_ATTEMPT` e HTTP 200; três acertos posicionaram a aula no **Item 16/20, progresso 75%**, em vez do Item 1. Status: **Nivelamento/Placement = OK_PRODUCTION**. No mesmo acabamento físico, o visual pedagógico de E1 foi aberto em tela cheia, permaneceu consultável, E1 avançou para E2 sem imagem própria, e o único novo T02/visual observado nos logs pertencia ao prefetch do item N+1, não à E2. Status: **Imagem/segundo professor = OK_PRODUCTION** para visual único em E1, consulta ampliada e ausência de duplicação em E2.

**ATUALIZAÇÃO 20 (Codex, mesma missão) — CG1, isolamento e replay econômico físico; identidade de restart corrigida**: troquei fisicamente da conta `aulasonline18` para a conta QA e o drawer mostrou somente as aulas da QA, sem qualquer aula recém-criada na conta anterior, confirmando segregação de estado no dispositivo e na projeção remota. Na aula CG1 de 60 itens da QA, retomei no Item 2/60, percorri E1 -> E2 sem robô ou nova T02 e avancei para o Item 3/60; os logs correlacionaram uma operação T02 para M0002 e uma para o N+1 M0003. O grant de teste foi feito pelo mecanismo canônico idempotente `TEST_CREDIT_EMAILS`/`bom_durable_grant_credits`, nunca por edição de saldo. A fonte durável mostrou saldo 999997, zero reservas e dois débitos após os dois packages. Um `force-stop` revelou um bug econômico real: o mesmo M0003 foi recomprado porque `accountGeneration` era um contador local de ativação (2 após account switch, 1 após restart), fragmentando a identidade paga; o saldo caiu indevidamente para 999996. O servidor `6885043` (`fix(cost): keep paid identity stable across restart`) removeu esse contador transitório das identidades T02/comercial/visual, preservando usuário+aula+item+conteúdo+locale+contrato, e adicionou uma ponte estritamente de leitura para resultados duráveis pré-migração. `check:protected` PASS; suíte servidor PASS **139/139**. Deploy exato, health 200, `NRestarts=0`. Reteste físico do mesmo Item 3/60: reabriu corretamente; houve apenas replay HTTP 200 em 487 ms com `duplicateSuppressed=true`, **zero `T02_STARTED`/`T02_ATTEMPT`**, saldo permaneceu 999996, revisão/transações permaneceram 42 e reservas permaneceram zero. Status: **CG1 = OK_PRODUCTION** para retomada e transição física inicial de currículo 60, apoiada pelas matrizes automatizadas até 400 itens; **Account isolation = OK_PRODUCTION** para troca física e segregação observada; **Microcrédito = OK_PRODUCTION** para grant canônico, captura por package, saldo, ausência de reserva órfã e replay de restart sem novo débito/provider.

**ATUALIZAÇÃO 21 (Codex, mesma missão) — Google Play Billing certificado fisicamente em produção**: a primeira tentativa de compra foi corretamente impedida porque o tablet ainda conservava o proxy `127.0.0.1:9` usado no ensaio offline. Depois de remover esse estado de rede de teste, o app abriu o checkout real da Google Play para `100 Learning Credits`, mostrou o preço regional `$1.99`, a forma `Test card, always approves` e a declaração de pedido de teste sem cobrança real. A compra foi aprovada pela Play; o servidor de produção recebeu uma única chamada `POST /api/play-billing/consume-credit-pack` e respondeu HTTP 200; o saldo visível passou exatamente de 999996 para 1000096. Após `force-stop` e reabertura, o app retomou a aula, consultou novamente o saldo e continuou mostrando 1000096, sem uma segunda chamada ao endpoint de grant. Status: **Billing = OK_PRODUCTION** para produto ativo, checkout Play físico, compra de teste aprovada, grant de 100 créditos, persistência e ausência de duplicação no restart. APP `2adc48e`, SERVER `6885043`, APK v110 SHA-256 `8f207e48a11d16fcca3589cf6e1db076f42adb143e7c192d2ace2cca3f011402`.

**ATUALIZAÇÃO 22 (Codex, mesma missão) — Finalização sem pending certificada fisicamente em produção**: criei pela UI uma aula nova e controlada para uma única habilidade estreita (identificar números pares e ímpares), sem material e começando do início. O T00 produziu legitimamente o mínimo normativo de 5 itens para uma habilidade; o aquecimento foi concluído e a aula abriu em Item 1/5. Percorri fisicamente E1 e E2 dos cinco itens, sempre com resposta correta e sinal 1 (`I am sure`), sem criar agravante, Amparo ou Recuperação. O progresso avançou uma vez por item até o Item 5/5. Depois da E2 final, o botão ainda rotulado `Continue to next item` abriu corretamente `Lesson completed`, barra 100%, `Your progress has been saved` e `Finish`; ao tocar `Finish`, o app retornou ao início do fluxo de nova aula. Não apareceu `Start recovery`, confirmando o caminho terminal limpo. Status: **Finalização sem pending = OK_PRODUCTION** no APP `2adc48e`, SERVER `6885043`, APK v110 SHA-256 `8f207e48a11d16fcca3589cf6e1db076f42adb143e7c192d2ace2cca3f011402`.

**ATUALIZAÇÃO 23 (Codex, mesma missão) — reidratação E2 e identidade econômica T02 fechadas fisicamente**: o APP `06b161f` corrigiu a retomada de E2 quando o package persistido já contém texto, mas não mantém um slot E2 duplicado; após `force-stop`, o Item 3/E2 reapareceu automaticamente. O SERVER `3628af9` acrescentou observabilidade segura à rota visual e foi implantado em produção com health/readiness HTTP 200. Na prova seguinte, o mesmo Item 4 foi chamado com corpos semanticamente diferentes pelo prefetch e pelo restart, produzindo chaves financeiras distintas e uma nova execução T02. A causa raiz era a expansão tardia de `profile` sobrescrever `item/objective` canônicos no cliente. O APP `2048697` passou a fazer os campos canônicos vencerem apenas no corredor normal, preservando os modos auxiliares `review/recovery/amparo/doubt`; teste de regressão compara prefetch e reidratação e exige os mesmos campos econômicos. Provas locais: `flutter analyze --no-pub` PASS; testes focados PASS **35/35**; suíte completa PASS **1506/1506**; governança e mutações OVERALL PASS no commit `b51df22`. APK v110 de produção instalado, SHA-256 `2038290591986f49568a0a303493955705c77f102bd5db2508a8719c98bc5f55`. Prova física no Samsung: entrada humana no Item 4/E2 preparou o Item 5 com `financialKey=55306e640ac8774d26c62bee`; depois de avançar ao Item 5, `force-stop` e reabertura emitiram a mesma chave, `duplicateSuppressed=true`, HTTP 200 em 247 ms e **zero `T02_STARTED`/provider/novo débito**. O Item 5/E1 reapareceu com texto, visual, pergunta e alternativas. As duas chamadas técnicas `/api/visual-route` do restore responderam 200 e não produziram evento financeiro/provider. Status: **Restart/Resume econômico T02 = OK_PRODUCTION**, sem recompra do package aceito.
...
| | SHA | branch | status |
|---|---|---|---|
| **BOM (app) main** | `b51df22` | main | pushado; último APK release instalado no tablet, SHA-256 `2038290591986f49568a0a303493955705c77f102bd5db2508a8719c98bc5f55` |
| **Servidor-BOM main** | `3628af9` | main | pushado, `git status` limpo |
| **Servidor de produção real (droplet)** | `3628af9` | — | rodando, `bom-api.service` active, health/readiness 200 |

Observação: o servidor de produção foi atualizado para o commit exato `3628af9` durante a instrumentação segura da rota visual; os releases anteriores `6885043`, `72c7d20` e `519f3c8` permanecem disponíveis para rollback.
...
| Anexos (geral) | OK_PRODUCTION | 1e7b20b | 519f3c8 | matriz física fechada em produção: TXT UTF-8, TXT UTF-16LE, PDF, DOCX e imagem aceitos e usados pedagogicamente |
...
| Imagens | OK_PRODUCTION | 1e7b20b | 519f3c8 | imagem controlada: T00 completo, aquecimento e aula `Who-What-Why` derivados da carta exibida |
| Imagem/segundo professor | OK_PRODUCTION | 2adc48e | 519f3c8 | visual pedagógico de E1 aberto em tela cheia; E2 sem imagem própria; novo visual nos logs correlacionado ao N+1, não à E2 — Atualização 19 |
...
| Nivelamento | OK_PRODUCTION | 2adc48e | 72c7d20 | três questões físicas em sequência; uma tentativa/HTTP 200 por questão; três acertos posicionaram no Item 16/20 (75%) — Atualização 19 |
| Placement (normal) | OK_PRODUCTION | 2adc48e | 72c7d20 | contrato package projetado corretamente; sem `AI_CONTRACT_INVALID` ou fallback silencioso — Atualização 19 |
| Placement (dentro de CG-1) | OK_PRODUCTION | dbe570d | 3628af9 | aula nova, objetivo amplo, "I already know something about this subject", 1 pergunta diagnóstica real sem AI_CONTRACT_INVALID, abriu em Item 1/120 (escala CG-1 real) |
| CG1 (currículo grande) | OK_PRODUCTION | 2adc48e | 6885043 | aula 60 itens retomada em 2/60; E1->E2 local; avanço e N+1 para 3/60; restart reidratou sem provider/débito; matrizes automatizadas cobrem fronteiras até 400 — Atualização 20 |
| **Amparo** | **OK_PRODUCTION** | 7665118 | 519f3c8 | oferta explícita restaurada; três estações concluídas fisicamente sem intro intermediária; retorno ao Item 4/20 comprovado — Atualização 13 |
...
| Recuperação | OK_PRODUCTION | 1e7b20b | 519f3c8 | produção real no Samsung: `Start recovery` somente na entrada; corredor completo até `Recovery completed`, sem cartão de preparação entre questões; contador convergiu de `1/7` a `1/1`; `Finish` retornou à aula em 100%; APK `d5c8c9f8...` |
| Finalização sem pending | OK_PRODUCTION | 2adc48e | 6885043 | aula controlada 5/5, dez experiências corretas+sinal 1, zero recovery; `Lesson completed` 100%, progresso salvo e `Finish` funcional — Atualização 22 |
| Finalização com pending | OK_PRODUCTION | 1e7b20b | 519f3c8 | Item 10/10 abriu o gate obrigatório; Recuperação concluiu e `Finish` retornou à aula com `100% Progress`; ver Atualização 12 |
...
| Rename | OK_PRODUCTION | dbe570d | 3628af9 | causa raiz real (race de stale_state_revision) corrigida com 1 retry limitado; 2 testes determinísticos novos + suíte 1508/1508; reteste físico numa aula com histórico real de sessões (QA-LiveRefresh-OK) |
...
| Offline/Reconnect | OK_PRODUCTION | 2adc48e | 519f3c8 | Q1 -> corte em Continue -> erro recuperável em 2/5 -> reconnect/retry direto para Q2 -> Q3, sem intro/robô, perda de índice ou novo provider duplicado; APK `8f207e48...`, ver Atualização 18 |
| Account isolation | OK_PRODUCTION | 2adc48e | 6885043 | troca física aulasonline18 -> QA mostrou somente aulas da QA; nenhuma aula da conta anterior vazou — Atualização 20 |
| Microcrédito | OK_PRODUCTION | 2adc48e | 6885043 | grant canônico; 1 crédito/package; zero reserva aberta; bug de identidade pós-restart corrigido; replay físico sem provider ou novo débito — Atualização 20 |
| Billing | OK_PRODUCTION | 2adc48e | 6885043 | checkout físico Google Play com produto ativo e cartão de teste; saldo 999996 -> 1000096; restart preservou grant sem segunda chamada — Atualização 21 |
